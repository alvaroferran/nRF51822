...
static void advertising_init(void){
    ...
    ble_uuid_t adv_uuids[] = {
                                {ORIENTATION_UUID_BASE    ,BLE_UUID_TYPE_BLE}
                            };
...
static void services_init(void){
    uint32_t       err_code;
    ble_orientation_init_t orientation_init;
    err_code = ble_orientation_init(&m_orientation, &orientation_init);
    APP_ERROR_CHECK(err_code);
}
...
static void ble_evt_dispatch(ble_evt_t * p_ble_evt){
    ...
    ble_orientation_on_ble_evt(&m_orientation, p_ble_evt);
}
